...
// **已知盲区两条**：① 压在贴图或九宫格板上的字算不出背板（板走 border-image），这一批单独用
// plates 模式列字色；② 半透明 background-color 只合成到"祖先链上第一个不透明色"为止，
// 底下若还铺着一张场景图（`.viewport` 的 hub_camp + 遮罩），那层不参与计算——所以量到的是
// 遮罩后的保守值，比真值略乐观。半透明面板改完仍要对着截图看一眼。
