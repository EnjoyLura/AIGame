...
ok('倒计时每秒刷新+关闭即停', /this\._stamTimer = setInterval\(\(\) => this\._stamTick\(\), 1000\)/.test(ui) && /isConnected\) \{\s*\n\s*clearInterval\(this\._stamTimer\);/.test(ui));
ok('广告条目：+10/剩余次数/满体力禁用', /看广告领体力[\s\S]{0,300}今日剩余 \$\{left\}\/3 次[\s\S]{0,400}left <= 0 \|\| full/.test(ui));
ok('钻石条目：+20/20💎/超上限囤积/余额禁用', /钻石购买[\s\S]{0,200}\+\$\{STAMINA_BUY_N\} 体力[\s\S]{0,100}可超出上限囤积/.test(ui) && /diamonds < STAMINA_BUY_COST/.test(ui));
ok('领奖/购买后就地重绘+顶栏刷新', /claimReward\('stamina'[\s\S]{0,400}this\._refreshTop\(\);\s*\n\s*this\._popRebuild\(opt\(\)\);/.test(ui) && /buyStamina\(STAMINA_BUY_N, STAMINA_BUY_COST\)[\s\S]{0,300}this\._refreshTop\(\);\s*\n\s*this\._popRebuild\(opt\(\)\);/.test(ui));
...
ok('出战体力不足守卫引导弹窗', /this\._openStaminaGate\(BattleConfig\.RUN_STAMINA_COST\)/.test(ui));
