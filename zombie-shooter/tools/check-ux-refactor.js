...
/* 五段顺序用 indexOf 链在 Core 上断言（CSS 里同名类会污染全文搜索）；
   CTA 段已是模板字符串（S 档 tail 变体），找 popCTA${ 模板头；
   不用 [\s\S]{0,N} 定长窗口——段内注释/分支一长大窗口就假 FAIL（2026-09-20 硬规矩） */
ok('五段式顺序 头/内容/消耗/CTA/槽位/底栏', (() => {
  const seq = ["'popScroll'", "'popCost'", "`popCTA${", "'popSlots'", "'popBar'"].map(s => core.indexOf(s));
