// 五界面 UX 重构断言：页面归属/入口接线/红点迁移/两层 CSS
const fs = require('fs');
const src = fs.readFileSync('assets/scripts/ui/HomeUi.ts', 'utf8');
let fail = 0;
const ok = (name, cond) => {
...
ok('_openStageRewardModal 存在', /private _openStageRewardModal\(\): void \{/.test(src));
...
// 4. 英雄页吸收技能
ok('_renderSkillCards 存在', /private _renderSkillCards\(def: HeroDef\): HTMLDivElement \{/.test(src));
...
ok('技能页三方法已删', !/private _buildSkillPage|private _refreshSkillPage|private _pickSkillHero/.test(src));
ok('技能页三字段已删', !/_skillPickEl|_skillListEl|_skillSelIdx/.test(src));
ok('_chestsEl/_lootPrevEl 字段已删', !/_chestsEl|_lootPrevEl/.test(src));

// 5. 玩法页：任务/签到 + 五大玩法入口
ok('_buildPlayPage 存在', /private _buildPlayPage\(root: HTMLDivElement\): void \{/.test(src));
...
ok('基地横幅不再含任务/签到按钮', !/questEntry"><\/button>` \+\s*\n\s*`<button class="btn gold sm signinEntry/.test(src.replace(/每日运营[\s\S]{0,600}/, '')));
