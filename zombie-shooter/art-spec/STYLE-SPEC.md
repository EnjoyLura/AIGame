...
### 7.4 一个主光：把世界打暖，不是把控件拉灰（2026-09-22 三十五轮，§4.9 第 4 条）

对标那条写的是"他们所有高光都是橙的，光源是脚下那块发光地板；我们冷蓝底 + 金强调 + 灰金属板，
三个色温打架"。**照着这句话动手会做错方向**，所以先量（`tools/audit_light_temp.mjs`，
口径与 §7.3 的审计一致：藏字截屏、按每族面的矩形取像素、算 R−B 当暖度、算上缘−下缘当光向）。

量出来三件事，第三件最反直觉：

1. **光向本来就是齐的**：五页里顶光 23 族、底光只有 1 族，而那唯一一族是船坞地面
   （`div.dk`，它自己就是发光体）。所以"统一光向"没活要干，写下来当规矩即可。
2. **打架的是两个极端的板，不是"三个色温"**：界面件整体落在暖度 +19~+44，
   只有金牌 CTA（+176）和蓝板（−66）跳在外面。中间那一大片其实是协调的。
3. **底图自己是接近中性的**：`scenes/hub_camp` 整张 +6、`scenes/bg_road` 整张 **−9**。
   也就是说控件没有错，**是它们站在一片没有光的地上**——界面件比自己的世界暖了二十多档。

**被否决的第一种做法：给两块极端的板加 filter 把它们拉回带宽。** 数算完了
（`tools/calc_plate_filter.py`，CSS 滤镜是逐通道闭式公式，可以在源件上直接算，不必构建一次试一档），
也渲染出对比图（`tools/preview_plate_filter.py`），眼睛直接否决：`saturate(.6)` 把主 CTA
从"最该抢眼的东西"拉成一块脏黄铜，**层级没了**；`sepia(.4)` 把蓝板拉到灰绿，
**"蓝 = 取消/次要"这个功能色没了**。两条都是数值合格、画面更坏。
**判据**：色温带宽不是目的，"一眼看出这是什么控件、该不该点"才是——
凡是为了让普查数字好看而牺牲辨识度的改法，一律先在源件上渲染出来看一眼再决定。

**做法改成给世界打主光**：在 `#homeUi` 的背景栈最上面加一层琥珀，用 `background-blend-mode: soft-light`
只与底图混合。强度是算出来的，不是试出来的——`tools/calc_key_grade.py` 把现有遮罩一起算进去，
七档候选里只有 **soft-light 琥珀 .45** 能把两张底图**都**送进 +19~+44，而亮度只从 33/29 抬到 42/38；
平涂同强度也能进带宽，但亮度抬到 55/52，等于把暗场景洗亮，会反过来吃亮字的对比度。

用 `background-blend-mode` 而不是盖一层 `div`，是因为混合只发生在本元素的背景栈内部，
**子节点（所有文字、所有板）完全不参与**——这一层在原理上就不可能动到可读性。
改完五页比值审计仍然不合格 0 个，护送页冷族从 2 归 0。

**剩下的一处不假装解决**：蓝板仍然 −49（压过之后）。整页主光照不到它（它是不透明贴图，压在底图之上），
而把它拉进带宽就等于丢掉它的功能色。这里只压了一档"去自发光"
（`#homeUi .blue.plated, #homeUi .diffSeg:not(.on).plated { filter: saturate(.82) brightness(.90) }`，
**不动色相**），目标是去掉"屏幕里另点了一盏冷灯"的感觉。
**为什么必须带上 `.diffSeg:not(.on)`**：护送页那两枚锁住的难度段用的是**同一张** `btn_cancel`，
但身上没有 `.blue` 类——只按类名写就会让同一张图在一处压过、另一处没压，两屏并排看成了"两种蓝"。
**判据**：按"哪张图"决定要不要压，不能按"哪个类名看起来像"；一张板铺在几个选择器上是常态
（`UiPlate` 的契约表就是干这个的），改图的观感时先回那张表查这张图还被谁用着。
**它仍然在带宽之外，这是有意的**，写在这里是为了让下一轮不要以为已经齐了。

### 7.5 采"字形的墨盒"，不是"元素的边框盒"（2026-09-22 三十七轮，斜切条族逼出来的）

`tools/contrast_audit.mjs` 一直取的是**元素**的 `getBoundingClientRect()`。在平涂圆角矩形上这没问题——
元素盒就是字所在的那块面。**平行四边形的板一起来，它就开始答错问题**：难度段那格实测元素盒 102×42，其中

- 两端各一个三角角口是**透过去的**，露出底下近黑的轨道（斜切必然如此，不是缺陷）；
- 板自己的暗底厚边占下沿一条；
- 真正压在"普通"两个字背后的，只有中间那块亮金。

结果均值被拖到 `#a76d25`、暗字算出 3.99:1 报 FAIL，而取景图里那两个字清清楚楚好读。
**这不是判据太严，是采样范围问错了问题**：WCAG 问的是"这些字压在它们背后那块东西上认不认得出"。

改法：`Range.selectNodeContents(e).getBoundingClientRect()` 取**墨盒**，取不到再退回元素盒。
阈值 4.5、大字号豁免线、inactive 另计、`plates` 那组高方差名单，**一条都没动**；
改完五页的元素计数仍是 47/42/49/32/43——没有任何位置因为这次改动从样本里消失。
**判据**：改口径之后如果计数会变少，那才是放宽；计数不变、只有比值变准，是修工具。

顺带一条同族的教训：**板面是中间调时，不存在好字色**。金板均值 `#976d25`，
亮奶油字 4.37:1、暗藏青字 3.28:1，两头都不达标——这时候在字色上打转是白打，
正解是把**牌面本身**推离中间调（`tools/tune_plate_face.py`：按亮度分位线性分配增益，
暗端不动、亮端吃满，所以描边与底厚边原样、只有牌面提亮，金属的明暗关系不被压平）。
选亮金 + 暗字而不是暗金 + 亮字，是因为"选中"这一档要和主 CTA 那块金牌说同一种话。
...
- **切片是行主序**（上到下、左到右）：prompt 里的格子顺序必须与之对应（写明「5列3行」「从左到右从上到下」），
  落盘前先肉眼核对切出的件与槽位一一对应；
- **板/条这类"要被九宫格切开拉伸"的件，prompt 必须写"哪一段不许变形"**（2026-09-22 r44 斜切条族）。
  图标族那条"提示词不替模型设计形象"的规矩**对板类不成立**：板的外形本身就是契约的一部分，
  中段只要不是等高的水平直带，横向一拉就露馅。所以这一族的 prompt 里明写了
  「斜切只发生在两端各一小段，中间那一段必须是上下边完全平行、高度恒定的水平直带——
  因为这条带在游戏里会被横向拉长到任意宽度；画成起伏/收腰/鼓肚/弧形整表作废」。
  同理"中央是一整块平整空白的牌面，不要镂空、不要挖洞、铆钉不许伸进牌面"也是板类专属要求。
  ⚠ 但**只写约束、不写审美**：斜度多少度、什么材质、几颗铆钉一律不给，给了就是又回到逐格设计形象。
...
| 列表行卡底板 | 商城货卡 `.good.panel`（`HomeUiMall.ts:304` 普通货卡 + `:496` 广告卡两处）｜战斗 HUD 邮件行 `.mailRow.panel`（`DomHud.ts:570`）｜弹层列表行 `.popRow`（`UiPlate.POP_PLATE` + `HomeUiCore._platePop`，2026-09-25 弹层美术架构轮接） | `ui/panel/row_card` | 卡片最小高 115px×--pu；HUD 那层 `--pu` 就是 `--s`；弹层行卡高约 54~60px | `nineSlice(el,'card')` | `card` = slice `24 fill` / width 12px×--pu | 1 | ✅ r16 表出图（两件焊死在一起，`--dilate 2` 才切得开）。**稀有度色标在内层 `.gIc` 的边框上、不在卡片本身**，所以贴板只吃掉卡片自己的中性边，不抢品质色。同批的 `panel_mini` 当时**没落盘**——当时写的理由是「跟 `.frame` 的装饰伪元素打架」，Step5 复核后**这条理由作废**：真正的理由是它的两个建点零调用（2026-09-25 起已拷回落盘并接弹层挑选格，结案见本节末「模块小框底板」那行）。`skillCard.panel` 也带 `frame`，同样跳过 |
...
| 斜切条族（r44 表 6 件，一张 3列2行出齐；2026-09-22 三十七轮，§4.9 第 5 条） | **`tools/audit_bevel_family.mjs` 实测**：顶栏资源胶囊 `.res::before` 三枚（140~161×29，画在**伪元素**上）｜难度段格 `.difficulty .diffSeg` 三枚（102~103×42）｜难度段轨道 `.difficulty`（310×44）｜章节标题条 `.chapter-head`（540×62，原先**完全没有底**）｜二级分类页签 `.flat-tabs > button`（135×47，由上一行**搬进来**，键没换）｜弹层页签 `.popTabs > div`（2026-09-25 起，走新档 `popTab`：同源件、显示宽度按宿主高 47→34 折算成 11.5/8px） | `ui/strip/res_band` `ui/strip/diff_off` `ui/strip/diff_on` `ui/strip/diff_track` `ui/strip/chapter_band` + 同名覆盖 `ui/tab/seg_plate` | 见右列四档 | `nineSlice` / **`nineSliceVar`**（`.res` 那条带在 `::before` 上，inline style 上不去，切片三件走自定义属性交给样式表消费） | ⚠ **这一族不许沿用上面任何一档的切法**：① 横向切片必须**整块包住斜边**（`tools/measure_slant.py` 实测六件斜边水平位移 52~70px），斜边被划进可拉伸中段就会"斜了一下又直了"；② 纵向切 **50%**，让整条只做等比纵向缩放，顶高光边与底 2.5D 厚边才不会被拉伸吃掉（这两条就是"厚度"的全部信息量）；③ 显示宽度要满足 `横向宽/纵向宽 ≈ 横向切片px/(源高/2)`，否则角区被非等比压扁、21° 会变成 11°。⇒ **一件一档**：`stripThin` 50% 72 / 10.5px+8.3px｜`stripCap` 50% 66 / 16px+11px｜`stripWide` 50% 70 / 22.5px+13.5px｜`stripTrack` 28% 56 / 6px+10px（轨道是空心凹槽，纵向不能切 50%，内壁要留在角区）。纵向一律按 **--pw** 不是 --pu（宿主高度本身按 --pw 缩放，而 `--pu` 只在 `#homeUi .btn` 上有定义、`.res`/`.diffSeg` 拿不到） | 2（难度段选中/未选两态**同形换色**，符合 §10） | ✅ 六件全部落盘接线，扫描走新表 `UiPlate.BEVEL_PLATE`（整 `#homeUi`，因为顶栏不在 `.viewport` 里）。`.diffSeg` 两行从 `CITY_BUTTON_PLATE`、`.flat-tabs > button` 一行从 `SURFACE_PLATE` **搬出**，并把 `#homeUi .blue.plated, #homeUi .diffSeg:not(.on).plated` 那条去自发光规则**缩回只管 `.blue.plated`**——难度段不再吃蓝板，留着会把中性暗金属往灰里再压一档。⚠ **`.res::before` 的 `skewX(-12deg)` 必须与上板同时撤掉**（写在 `.res.plated::before` 里）：斜度从此由图带，CSS 再斜一次就是斜上加斜、角度翻倍。缺图回退时 `.plated` 不存在，旧的平涂斜带原样保留。**显式跳过 1 处**：关卡说明条 `.stage-caption` 实测只有 374×22，`stripCap` 上下板厚就要 22px，牌面归零必压字，表里写 `key: null`。**这一族只收"条"**：里程碑/编队/出战底栏/日常四格/副本行/远征行实测高 80~115，是容器不是条，归 §4.9 第 2 条 |
| 选中抬起件族（r45b 表 2 件，一张 1列2行出齐；2026-09-24 三十九轮，§4.9 第 6 条） | 宿主裁定与普查实测见 §10.1（6 处 `.on` 面裁成 3 处）：底部主导航 `.tab.on`（102×93）｜难度段选中档 `.difficulty .diffSeg.on`（102×42）｜二级分类页签选中档 `.flat-tabs > button.on`（130~135×46）｜弹层页签选中档 `.popTabs > div.on`（约 34px 高，2026-09-25 起，`UiPlate.POP_PLATE`） | `ui/strip/tab_raised`（近方瓷砖 733×685）、`ui/strip/band_raised`（斜条 1510×404，两处选中签共用一件） | — | `nineSliceVar`（`.tab.on` 的瓷砖画在 `::before` 上——整条 .tabbar 是一块板，格内换板没地方画，随宿主 translateY(-4px) 整体抬起）/ `nineSlice`（难度段与页签选中档直接 border-image，translateY(-3px)） | `tabRaised` 28 fill / 4px｜`bandRaised` 50% 110 / 21px+8px（斜边 87×1.25≈110；纵向 50% 同斜切条族口径） | 每宿主 1（选中态专件；未选中态仍用各自原板） | ✅ r45b 一次过检（第一版 2 格因画出底座托槽 + 端头六边形整表作废，两条硬约束固化进 §8 与 §10.1）。**换的是形状不是颜色**：`.tab.on` 的凹下底色与金槽、`.flat-tabs button.on` 的金线与金三角都在 `.plated` 时撤掉；`.diffSeg.on` 的亮金板 diff_on 退役（图留库，归宿记在 §10.1）。选中段字色翻亮（gold-hi）同样挂 `.plated`，缺图回退旧配色原样保留 |
| 二级弹层族（`UiPlate.POP_PLATE` 新表，2026-09-25 弹层美术架构轮） | **普查工具 `tools/probe_popups.mjs` 六面实测**（邮箱/公告/设置/资料/体力/建筑详情）：弹层的中段组件当时全是平涂，面板底/横幅/品质头框/CTA/关闭钮此前已接。本族宿主：列表行 `.popRow`｜页签 `.popTabs > div`（含选中档 `.on`）｜挑选格 `.popGrid > i`（panel_mini 第一个活宿主）｜材料槽 `.popSlots .sc` | `ui/panel/row_card`、`ui/tab/seg_plate`、`ui/strip/band_raised`、`ui/panel/panel_mini`、`ui/panel/eq_slot`——**全部沿用已落盘件，本族零新出图** | 行卡高约 54~60px｜页签约 34px｜挑选格 c4 档约 78px 方｜材料槽 50px | `nineSlice`，扫描走 `HomeUiCore._platePop`（每次 `_renderPop` 末尾对当前 `.pop` 重扫——弹层是重绘的临时 DOM，切页后才跑的页面级扫描器扫不到它） | 行卡 `card`｜页签未选 `popTab`（seg_plate 源件、显示宽度 11.5/8px 按 47→34 折算，D21「切片不变、显示宽度跟宿主走」）｜页签选中 `bandRaised`｜格与槽 `sq` | 选中态（格 `.sel` / 槽 `.on`）走 CSS inset 金圈，不出二态图（§10） | ✅ 四族五行全接已落盘件。**显式跳过**（判据记在 POP_PLATE 注释里）：`popKV`/`popAttr`/`popSec`/`popMeta`（文本行族，每行一块板会把阅读面切碎）、`popCost .ci`（40px 图标格贴 sq 档剩 16px 内腔）、`popChip`（板厚四倍于胶囊高）、`popCard`（L5 演出卡随第二轮）。字色翻亮全部挂 `.plated`（HomeUiStyle 末段「二级弹层族」那块） |
| 容器底板族（r31 表 7 件，一张 3列2行出齐） | **实测宿主 21 个面**：底部主导航 `.tabbar .tab` 五格（106×93，每页都在屏，是普查里最大的一片无图面）｜二级分类页签 `.flat-tabs > button`（商城 4 签 + 背包 4 签共用，135×47）｜战斗 HUD 功能钮 `.hudBtn` 三枚（菜单 64×47、暂停与战报 53×47）｜英雄页装备六槽 `.eqGrid .slot`（82×83）｜弹层材料槽 `.popSlots .sc`（50px 高，2026-09-25 起，跨族共用这张已落盘件）｜基地页建筑卡 `.building` 八张（251×150，**这一页原先九宫格数为 0**）｜行动页模式入口卡 `.challenge-ground .entry` 两张（240×235） | 导航格 `ui/nav/tab_plate`（**整条 `.tabbar` 一块**，见右列末注；`ui/nav/tab_plate_on` 已随之下架，图在 `art-spec/reference/stock/nav/`）｜页签 `ui/tab/seg_plate`（**2026-09-22 三十七轮搬入下面「斜切条族」**，键不变、切片档换成 `stripCap`）｜HUD 钮与装备槽共用 `ui/button/btn_hud` 加 `ui/panel/eq_slot`｜大卡 `ui/panel/building_card` `ui/panel/entry_card` | 按宿主分四档，见右列 | `nineSlice`，扫描走 `UiPlate.SURFACE_PLATE` 这张**新表**（不并进 `CITY_BUTTON_PLATE`）：按钮族只扫 `.viewport`，而导航格在 `.viewport` 之外的 `.tabbar` 里，容器族必须扫整个 `#homeUi`，所以本表选择器一律自带容器前缀。四档切片全用百分比（同族源件比例从 6:1 的薄条到 1.2:1 的方卡，像素档一档通用不了） | `navTab` 26% 12% 与 10px×--pu（导航格那枚图标本身就有 102 见方，板只能露出四边一圈，再厚就把图标压进包边）｜`seg` 30% 10% 与 9px（最薄的条，上下给到 30% 才包得住两头角码，左右再宽就没平牌面了）｜`sq` 26% 22% 与 12px（近方小件：HUD 钮 + 装备槽）｜`bigCard` 26% 12% 与 15px（宿主 ≥150 高的大面） | 导航格 1（**2026-09-21 由 2 改回 1**，见右列）；其余 1 | ✅ 七件全部落盘接线，`--tight` 切片、`trim_alpha.py --check` 七件透明边均 ≤6%（这批是本族第一次用紧切，方化画布那个坑见本节末「板件不许留透明边」）。⚠ HUD 三枚与装备六槽是**两块不同的板**而不是同一块：出图时模型给了两枚近方件，正好按宿主分给两处，不做复用。**⚠ 导航格已于 2026-09-21 反档**：五格各贴一块板，板与板之间必然留缝，读成"五个独立按钮浮在一条带上"——正是用户点的"方块感"。改成整条 `.tabbar` 贴一块、复用同一张图的 `navTab` 档（中间区横向拉长就是连续板面，铆钉只落在整条两端），`SURFACE_PLATE` 两行并成一行 `{ sel: '.tabbar', key: 'ui/nav/tab_plate' }`。**代价**：一块板上没有"哪一格换了板"这回事，选中态不能再靠换板表达，改由 CSS 三件事承担——凹下的深色底（`rgba(4,9,14,.42)` + inset 阴影）、顶部一枚金槽（`.tab.on::after`）、字色与图标转金。选中的那一格因此**不再是"没有图的位置"**，它只是不用自己的图 |
...
| 模块小框底板 1 件（**死方法里的引用不算宿主**——教训保留） | ~~`.mbox.frame` 两处（`_openSheet`/`_openResult`，零调用桩子）~~ → **弹层挑选格 `.popGrid > i`（`UiPlate.POP_PLATE`，2026-09-25 起）** | `ui/panel/panel_mini` | 挑选格约 78px 方（c4 档）×--pu | `nineSlice(el,'sq')` | `sq` = slice `26% 22% fill` / 12px×--pu | 1 | ✅ **2026-09-25 结案**：r16 表件（金属包边 + 四角铆钉 + 平整内芯）在 `art-spec/reference/stock/panel/` 归档躺了很久，弹层美术架构轮给 `.popGrid > i` 接板后按「拷回即生效」落盘进包，RESERVED 声明同步移除（`check-art-manifest` 盯的就是这条）。历史判据原样保留：`.mbox` 看着是活的 CSS 类（还有 giftBox/questBox/lbBox/setBox 一堆变体），实际全工程只有那两个零调用方法建它，它们是「全部二级界面已迁新弹层」那条迁移棘轮的桩子（`check-ux-refactor` 断言调用点必须为 0、`check-ux-layout-bundle` 断言方法还在包里，删不得也用不上）——**在死方法里加一行 `nineSlice`，「在库件必须有归宿」照样绿灯，图却永远上不了屏**。核宿主先核「谁调用这个建点」，不是「哪里提到这个 key / 哪里有这条 CSS」；`.mbox/mHead/mClose/mSub/mRow` 整族列入死样式清扫那一轮 |
...
| 页签/卡片选中 | 同件 + `transform: scale(1.1)` + 选中底色/内阴影（`.tab.on` / `.popTabs .on`） | CSS，不出 `nav_*_on` 二态图。**§4.9 第 6 条要翻掉这一行**，见下面「选中态抬起」那一段 |
...
### 10.1 「选中态抬起」的宿主裁定（2026-09-22 三十八轮裁定；2026-09-24 三十九轮 r45b 出图进版，§4.9 第 6 条）

上面「页签/卡片选中 = 同件换色」那一行是**撤板轮**定的，对标之后判定它不够：商业款把选中那一格
**抬起来成另一个轮廓**（一方瓷砖升起 + 顶上一道标头），换的是形状不是颜色。所以这一条要出第二件，
但**只给真的在屏上"多选一"的位置出**——普查（`tools/audit_bevel_family.mjs` +
`tools/audit_ui_surfaces.mjs`，逐页找类名带 `on`/`active` 的面）实测如下：

| 位置 | 实测 | 屏上有几个"选中" | 裁定 |
|---|---|---|---|
| 底部主导航 `.tab.on` | 102×93 | 5 选 1 | ✅ **要出**（对标截图里就是这一格，这一族的旗舰） |
| 难度段 `.diffSeg.on` | 102×42 | 3 选 1 | ✅ **要出**（现在靠亮金板换色表达，正是这一条要翻掉的） |
| 二级分类页签 `.flat-tabs button.on` | 130~135×46 | 4 选 1 | ✅ **要出**（现在只靠顶部金标 + 底部金线，最弱的一处） |
| 英雄编队行 `.hpick.on` | 127×47 | 5 选 1（`HomeUiHeroes.ts:458` 按 `_heroSelIdx`） | ❌ **本轮不出**：它未选中那档还在**按钮族**（`ui/button/btn_row` 圆角板）。选中档若换成斜切抬起条，同一个位置两态就分属两种外形语言，读起来像换了个物种。先把 `.hpick` 整族搬进斜切条族，再谈抬起——那是另一轮 |
| 编队号位 `.squadEntry.on` | 40×39 | **可以四个同时 on** | ❌ **不出**：`HomeUiHeroes.ts:688` 里 `on` 的条件是"这一档有配置"、`active` 是"这个英雄在阵容里"，**都不是"当前选中"**。给非选择语义贴"选中件"，等于让四个格子同时都声称自己是当前项 |
| 弹层列表行 `.expRow.on` `.dgRow.on` `.bondRow.on` | 未在本轮普查快照里出现（弹层未开） | — | ❌ **暂不出**：它们在浅纸弹层里，浅纸是另一套底（§7.1），抬起件用的暗金属放进去会压不过纸白。要出先单独裁一次弹层族的选中语言 |

**两条出图硬约束（第一版 2 格整表作废换来的，写进 §8 那一族的要求里）**：

1. **不许把它所在的那条底板/凹槽/托盘一起画出来**。第一版给每件都连画了一截横底座，"抬起"确实读出来了，
   但那截底座在游戏里是**另一件已经存在、且会被横向拉伸到任意宽度**的贴图（`.tabbar` 整条一块板、
   `.difficulty` 一条轨道），画死在这一格里就永远对不上长度。"抬起"只许靠**贴在本体自己底沿外侧的
   一小条投影**表达。
2. **斜切端头会被默认画成六边形**（上下各切一角）。上一族定下来的是**平行四边形**——端边一条从顶角
   直斜到底角。这两个形状肉眼都叫"斜切"，并排放就露馅。所以"斜切"这个词不够，必须写成
   「端边是一条从顶角一直斜到底角的完整直线，不许上下各切一角、不许切角、不许端边垂直」。

**进版记录（2026-09-24 三十九轮 r45b）**：换新网关后按上面两条硬约束重出，1列2行表 2 件**一次过检**
（近方瓷砖无底座托槽、只带本体底沿一小条投影；横条端边为顶角到底角的完整斜线、中段等高），
`tools/measure_slant.py` 实测斜边位移 87px、瓷砖倒角 19px，据此定两档：
`tabRaised` 28 fill / 4px、`bandRaised` 50% 110 / 21px+8px（斜边 87×1.25≈110 进角区，纵向沿用本族 50%）。
三处✅宿主全部接线：`.tab.on` 画在 `::before`（`--tab-raised`）随宿主 translateY(-4px) 抬起，
凹下底色与顶沿金槽在 `.plated` 时撤掉；`.diffSeg.on` 与 `.flat-tabs > button.on` 直接 border-image、
translateY(-3px)，后者的金线与金三角同样在 `.plated` 时撤掉。**r44 的亮金板 `ui/strip/diff_on`
从本条起退役**：不再被任何契约表引用，图留在库不删盘，本节即它的归宿记录；选中段的字色由
navy（压金板）翻回 gold-hi（压暗牌面），同样挂在 `.plated` 上，缺图回退时旧配色原样保留。
