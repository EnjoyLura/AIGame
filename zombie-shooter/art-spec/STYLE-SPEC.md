...
变成了这一页继续加美术的前置条件。**这条已在三十三轮补掉，见 §7.3。**

### 7.3 可读性审计改成"看屏幕上真的那块颜色"（2026-09-22 三十三轮，#44 结掉）

§7.2 末尾那条盲区不是"量不到所以保守一点"，是**量错**。旧审计器沿祖先链找第一个不透明
`background-color` 当背板，于是整页底失效的那几轮里，它仍然能在 `#homeUi` 上找到 `--c-scene-1`
那块假想底色，照着它算出"不合格 0"，而玩家看到的字压在一张照片上。**工具报绿、画面是坏的**，
这是最坏的一种工具。所以这一轮不往全屏再加美术，先把眼睛修好。

新做法（`tools/contrast_audit.mjs` 整个换掉）：

1. 收集页面上每个"自己有文本节点"的元素，记下字色、祖先连乘透明度、矩形；
2. 注入一条覆盖样式把**字形**擦掉（`color: transparent` 还不够，彩色字形只吃
   `-webkit-text-fill-color`），版面与背板原样保留；
3. `Page.captureScreenshot` 截一张，把图送回页面画到 canvas，逐元素按矩形取**均值当背板**、
   **标准差当"这块背景有多花"**；
4. 比值在 Node 侧算，阈值与"inactive controls 豁免"口径与旧版一字未改。

三条实测结论，都直接影响后面怎么画：

- **纯 emoji 的位置不进字色统计**。emoji 字形自带颜色，CSS 的 `color` 对它没有意义，
  旧版把顶栏三枚按钮判成"黑字压暗底 1.17:1"就是这么来的。现在按
  `\p{Extended_Pictographic}` 整串匹配把它们分到"图形符号位"另报——**这一批的判据是"该不该出图"，
  不是对比度**，正好是 §4.9 第 3 条（一套图标语言）的采购单。
- **`--c-text-dim` 这个 token 配的是平涂冷底，照片底上不够亮**。它原先是 `#8ba3c7`，
  对着 `--c-scene-*`（`#1b2430`）有 6.08:1；整页底换成营地照片后，页面上最亮的落字带实测到
  `#554a40`，同一个值只剩 3.35:1。改成 `#a3bad4`（同一 token 单源改值，125 处用法一起动，
  不逐处补），并把 `--c-text-dim` 与 `--c-text-mute` 的间距留住——**翻亮的是次要文字，
  不能把层级翻平**。改完五页不合格从 9 归 0（213 个有字元素全量）。
- **字压在照片亮带上，正解是给那一块自己的面，不是把字再翻亮**。英雄页"战力 800 ⓘ"就是例子：
  它坐在照片里最亮的那条帐篷暖带上。给整行补一层**径向**压暗（不是圆角矩形——清方块感那一轮
  刚把它的橙色药丸拆掉，画个框回去等于推翻那轮的决定），ⓘ 那枚按钮另外给一块深色圆盘，
  因为它挂在盒子 `right:0`、正好在径向淡出之外。**判据**：一块底撑不住一排字时，
  先看这些字是不是坐在同一个面上；控件（按钮/信息牌）永远可以有自己的面，正文不行。

还有一条与本节并列的：**中亮度板不许沿用暗字**。按钮族原先只有两档——金牌（`#f29f0a`，暗字对）
和蓝板（`#35373c`，亮字对）。紫板 `btn_purple` 实测 `#743884` 卡在中间：`.major.plated` 那条
"金字翻暗"的规则套上去，2.28:1，比不翻还糟。**新增一档就新增一条翻亮规则**，
写在 `.major.plated` 之后（`#homeUi .game-button.purple.plated`，特异性 (1,4,0) 压过它）。

用法变化：第 4 个参数 `plates` 现在打的是**背板标准差 >28 的那批字**（均值达标但底下是一张照片，
局部笔画可能正压在亮块上——这才是真正需要人看一眼的），不再是旧版那句"算不出背板、需肉眼判"。
