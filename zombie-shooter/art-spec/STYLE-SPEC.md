...
| 战斗表现件余 5 件（特效 4 / 激光束）——**不是缺图，是缺表现代码** | 无：全工程 grep 连拼 key 的地方都没有（结算面是 DOM 弹层；升级走横幅 + 画布施法环；波次衔接是代码淡入淡出；飘字是 Label 直接画字；laser 英雄在 `HeroCombat` 的 `visualKey` 三元里根本没有分支） | `fx/coin_burst` `levelup_glow` `portal` `dmg_word`、`weapons/laser_beam` | 无 | — | — | — | ⏸ **整批不排产**，键与说明留在 `RESERVED_SLOTS`（每条都写了"为什么现在出图也没人读"）。这五件的正确顺序是**先写表现代码、再出图**——先出图只会得到五个躺在包里没人引用的文件（正是 D6 那条「预载但不引用」要防的东西）。想清账有两条路：给它们建消费点（结算金币爆开 / 升级光柱 / 侧翼切入传送门 / 暴击飘字底纹 / 激光束叠图），或者撤键。撤键要用户点头，本轮不动 |
