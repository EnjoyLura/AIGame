...
| 战斗表现件 9 件（关卡背景 4 / 特效 4 / 激光束）——**不是缺图，是缺表现代码** | 无：全工程 grep 连拼 key 的地方都没有（战斗底图只有 `scenes/road` 一个消费点；结算面是 DOM 弹层；升级走横幅 + 画布施法环；波次衔接是代码淡入淡出；飘字是 Label 直接画字；laser 英雄在 `HeroCombat` 的 `visualKey` 三元里根本没有分支） | `scenes/bg_forest` `bg_beach` `bg_snow` `bg_cave`、`fx/coin_burst` `levelup_glow` `portal` `dmg_word`、`weapons/laser_beam` | 无 | — | — | — | ⏸ **整批不排产**，键与说明留在 `RESERVED_SLOTS`（每条都写了"为什么现在出图也没人读"）。这九件的正确顺序是**先写表现代码、再出图**——先出图只会得到九个躺在包里没人引用的文件（正是 D6 那条「预载但不引用」要防的东西）。想清账有两条路：给它们建消费点（按章换战斗底图 / 结算金币爆开 / 升级光柱），或者撤键。撤键要用户点头，本轮不动 |
