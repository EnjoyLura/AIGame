...
| 同上 | `ui/ico/tab_core` | ⏸ 图合格但无宿主 | 规范里那第三个宿主 `.chTabs`（章节页签）**不存在**：样式还留在 `HomeUiStyle.ts:758`，DOM 早在护送页改版时换成「章节头 + 场景内侧左右翻页箭头」（`HomeUiStage.ts:68` 注释即「替代原五章页签」）。背包也没有核心分类，武器核心只是养成弹窗里的一行。**键留在采购单、切片件不落盘**（落盘就会破「在库件必须有归宿」那条断言），等真出现核心分类页签再贴 |
