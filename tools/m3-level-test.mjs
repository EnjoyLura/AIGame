...
assert.match(componentSource, /vx: number;/);
assert.match(componentSource, /vy: number;/);
assert.match(componentSource, /projectile\.target = null;\s*projectile\.x \+= projectile\.vx \* projectile\.speed \* deltaTime;/);
assert.match(componentSource, /private hasLeftBattlefield\(projectile: Projectile\): boolean/);
